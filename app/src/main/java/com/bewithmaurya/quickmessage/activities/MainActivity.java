...
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_MESSAGE;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_RECEIVER_ID;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_RECEIVER_IMAGE;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_RECEIVER_NAME;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_SENDER_ID;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_SENDER_NAME;
import static com.bewithmaurya.quickmessage.utilities.Constants.KEY_TIMESTAMP;
...
import com.bewithmaurya.quickmessage.adapters.RecentCoversationAdapter;
import com.bewithmaurya.quickmessage.databinding.ActivityMainBinding;
...
public class MainActivity extends BaseActivity implements ConversionListener {
    ...
    private RecentCoversationAdapter conversationAdapter;
    ...
    private void init()
    {
        ...
        conversationAdapter = new RecentCoversationAdapter(conversation,this);
    ...
    private void listenerConversation()
    {
        ...
        database.collection(Constants.KEY_COLLECTION_CONVERSATION)
                .whereEqualTo(KEY_RECEIVER_ID,preferenceManager.getString(Constants.KEY_USER_ID))
                .addSnapshotListener(eventListener);
    }
            ...
            for(DocumentChange documentChange : value.getDocumentChanges()) {
                ...
                else if(documentChange.getType() == DocumentChange.Type.MODIFIED)
                {
                    for (int i = 0; i < conversation.size(); i++)
                    {
                        String senderId = documentChange.getDocument().getString(KEY_SENDER_ID);
                        String receiverId = documentChange.getDocument().getString(KEY_RECEIVER_ID);
                        if(conversation.get(i).senderId.equals(senderId) && conversation.get(i).recieverId.equals(receiverId))
                        {
                            conversation.get(i).message = documentChange.getDocument().getString(Constants.KEY_LAST_MESSAGE);
                            conversation.get(i).dateObject = documentChange.getDocument().getDate(KEY_TIMESTAMP);
                            break;
                        }
                    }
                }
            }
            Collections.sort(conversation, (obj1, obj2) -> obj1.dateObject.compareTo(obj2.dateObject));
